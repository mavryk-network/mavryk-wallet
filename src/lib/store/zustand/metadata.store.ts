...
export const metadataStore = createStore<MetadataStore>()(
  persist(
    (set) => ({
      ...
      // Token actions
      putTokensMetadata: (records, resetLoading) => set((state) => {
      ...
      addWhitelistTokensMetadata: (tokens) => set((state) => {
        const updated = { ...state.tokensMetadata };
      ...
      refreshTokensMetadata: (records) => set((state) => {
      ...
      putCollectiblesMetadata: (records, resetLoading) => set((state) => {
      ...
      putRwasMetadata: (records, resetLoading) => set((state) => {
      ...
      putTokenMetadataDirectly: (slug, metadata) => set((state) => ({
        tokensMetadata: { ...state.tokensMetadata, [slug]: metadata }
      })),

      putCollectibleMetadataDirectly: (slug, metadata) => set((state) => ({
...
// Typed selector hook
export const useMetadataStore = <T>(selector: (state: MetadataStore) => T): T =>
  useStore(metadataStore, selector);

// Convenience selectors — match the old Redux selector hook signatures

// Tokens
export const useTokenMetadataSelector = (slug: string) =>
  useMetadataStore(s => s.tokensMetadata[slug]);

export const useAllTokensMetadataSelector = () =>
  useMetadataStore(s => s.tokensMetadata);

export const useTokensMetadataLoadingSelector = () =>
  useMetadataStore(s => s.tokensMetadataLoading);

// Collectibles
export const useCollectibleMetadataSelector = (slug: string) =>
  useMetadataStore(s => s.collectiblesMetadata[slug]);

export const useAllCollectiblesMetadataSelector = () =>
  useMetadataStore(s => s.collectiblesMetadata);

export const useCollectiblesMetadataLoadingSelector = () =>
  useMetadataStore(s => s.collectiblesMetadataLoading);

// RWAs
export const useRwaMetadataSelector = (slug: string) =>
  useMetadataStore(s => s.rwasMetadata[slug]);

export const useAllRwasMetadataSelector = () =>
  useMetadataStore(s => s.rwasMetadata);

export const useRwasMetadataLoadingSelector = () =>
  useMetadataStore(s => s.rwasMetadataLoading);
