...
Given(
  ...
  async (key: IEnterValuesKey, elementName: string, pageName: string) => {
    const inputText = requireE2eValue(key, iEnterValues[key]);
...
Given(/I have imported an existing account/, { timeout: LONG_TIMEOUT }, async () => {
  const seedPhrase = requireE2eValue('DEFAULT_HD_ACCOUNT_SEED_PHRASE', BrowserContext.seedPhrase);
  const password = requireE2eValue('DEFAULT_PASSWORD', BrowserContext.password);
  ...
  await Pages.ImportExistingWallet.enterSeedPhrase(seedPhrase);
  await Pages.ImportExistingWallet.nextButton.click();

  await Pages.SetWallet.isVisible();
  await Pages.SetWallet.passwordField.type(password);
  await Pages.SetWallet.repeatPasswordField.type(password);
